...
create table ware(w_id int not null
, w_ytd decimal(12,2)
, w_tax decimal(4,4)
...
, w_zip char(9)
, primary key(w_id)
)tablegroup tpcc_group partition by hash(w_id) partitions 6;
...
)tablegroup tpcc_group partition by hash(d_w_id) partitions 6;
...
)tablegroup tpcc_group USE_BLOOM_FILTER = TRUE partition by hash(c_w_id) partitions 6 ;
...
, h_data varchar(24)
)tablegroup tpcc_group partition by hash(h_w_id) partitions 6;
create table nord (no_w_id int
, no_d_id int
, no_o_id int
, primary key ( no_w_id, no_d_id, no_o_id )
)tablegroup tpcc_group  USE_BLOOM_FILTER = TRUE partition by hash(no_w_id) partitions 6;
...
)tablegroup tpcc_group BLOCK_SIZE=8192 USE_BLOOM_FILTER = TRUE partition by  hash(o_w_id) partitions 6;
...
, i_data varchar(50)
, i_im_id int
, primary key(i_id))   BLOCK_SIZE=16384
